...
ground_size = map.km_to_world(0.01) # METERS

def paintGL():
    global counter
    
    counter = (counter + 5) % 360
    ...
    gluPerspective(90, window_width / window_height, 0.0001, 10000)
    ...
    glTexCoord2f(-100 / ground_size, -100 / ground_size)
    glVertex3f(-100, -100, -0.00001)
    ...
    if map.origin:
        map.origin.draw(counter)

    if map.destiny:
        map.destiny.draw(counter)

    # DRAW BUTTONS
    for button in map.buttons:
        button.draw(window_width / window_height)
