...
def paintGL():   
    ...
    if len(map.line_strings.get('path', [])) > 0:
        gluPerspective(45, glutGet(GLUT_WINDOW_WIDTH) / glutGet(GLUT_WINDOW_HEIGHT), 0.02, 1000)
    ...
    for type in map.polygons:
        for polygon in map.polygons[type]:
            ...
            glRasterPos2f(polygon.center.x - text_size.x / 2, polygon.center.y - text_size.y / 2)
            
            for c in polygon.name:
